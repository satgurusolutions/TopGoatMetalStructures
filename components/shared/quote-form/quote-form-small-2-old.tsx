...
const formSchema = yup.object({
  ...
  BuildingDescription: yup.string().default(''),
...
export default function QuoteFormSmall2({
  isMilitary,
  leadSourceName,
  products,
  selectedProductId,
  ...
  const form = useForm<QuoteFormValues>({
    ...
    defaultValues: {
      ...
      State: '',
      Zip: '',
      BuildingDescription: selectedProductId,
      BuildingStyle: '',
      Notes: '',
      LeadSource: leadSourceName || 'Website Get a Quote Form',
      MilitaryBranch: 'Navy',
      Terms: false,
      BuildingStyleOther: '',
      EstimatedBudget: '',
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="pt-2 pb-4 text-center font-bold">OR</p>
        <Controller
          ...
          render={({ field }) => (
            <div>
              ...
              {form?.formState?.errors?.BuildingDescription && (
                <p className="mt-1 text-xs text-red-500 lg:text-sm">
                  {form?.formState?.errors?.BuildingDescription.message}
                </p>
              )}
            </div>
          )}
